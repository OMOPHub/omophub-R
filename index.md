...
Access SNOMED CT, ICD-10, RxNorm, LOINC, and 90+ OHDSI ATHENA
vocabularies without downloading, installing, or maintaining local
databases.

[![CRAN
status](https://www.r-pkg.org/badges/version/omophub)](https://CRAN.R-project.org/package=omophub)
[![CRAN
downloads](https://cranlogs.r-pkg.org/badges/omophub)](https://cran.r-project.org/package=omophub)
...
**[Documentation](https://docs.omophub.com/sdks/r/overview)** · **[API
Reference](https://docs.omophub.com/api-reference)** ·
...
Working with OHDSI ATHENA vocabularies traditionally requires
downloading multi-gigabyte files, setting up a database, and writing
complex SQL queries. **OMOPHub eliminates this friction.**
...
Get your API key from the [OMOPHub
Dashboard](https://dashboard.omophub.com/api-keys).
...
[`tibble`](https://tibble.tidyverse.org/) with one row per input
...
For pipe-friendly workflows, three standalone wrappers forward to the
...
Both forms are fully supported - pick whichever reads better for the
surrounding code.

### FHIR Client Interop

When you need raw FHIR `Parameters` / `Bundle` responses instead of the
Concept Resolver envelope,
[`omophub_fhir_url()`](https://omophub.github.io/omophub-R/reference/omophub_fhir_url.md)
returns the OMOPHub FHIR Terminology Service base URL for a given FHIR
version (`"r4"` default, plus `"r4b"`, `"r5"`, `"r6"`). Use it with
[`httr2`](https://httr2.r-lib.org/) or
[`fhircrackr`](https://cran.r-project.org/package=fhircrackr) to talk
...
**When to use which**: Use `client$fhir$resolve()` (or
[`fhir_resolve()`](https://omophub.github.io/omophub-R/reference/fhir_resolve.md))
when you want OMOP-enriched answers (standard concept, CDM target table,
mapping quality). Use
[`omophub_fhir_url()`](https://omophub.github.io/omophub-R/reference/omophub_fhir_url.md) +
`httr2` when you need raw FHIR responses for FHIR-native tooling.
...
Map several native codes in one request with `client$mappings$map()`.
Inputs that do not produce a mapping are preserved in `unmapped_sources`
with a `source_not_found` or `no_mapping_found` reason.
...
| `mappings` | Cross-vocabulary mappings | [`get()`](https://rdrr.io/r/base/get.html), `get_all()`, [`map()`](https://purrr.tidyverse.org/reference/map.html) |
...
HTTP errors are raised as [httr2 condition
classes](https://httr2.r-lib.org/reference/req_error.html)
(`httr2_http_404`, `httr2_http_401`, `httr2_http_429`, `httr2_http_403`,
etc.). Pre-request input-validation errors use the SDK’s
...
See
[`inst/examples/error_handling.R`](https://omophub.github.io/omophub-R/inst/examples/error_handling.R)
for the full set of patterns including graceful degradation and batch
error collection.
...
MIT License - see [LICENSE](https://omophub.github.io/omophub-R/LICENSE)
for details.
